...
(function(){
  ...
  const el = {
    ...
    dropzoneInput: document.querySelector('[data-dropzone-input]'),
    uploadBtn: document.querySelector('[data-action="upload"]'),
  ...
  function bindEvents() {
    ...
    if (el.dropzone) {
      ...
      el.dropzone.addEventListener('keydown', handleDropzoneKeydown);
    }
    if (el.dropzoneInput) {
      el.dropzoneInput.addEventListener('change', handleFileSelect);
    }
    if (el.uploadBtn && el.dropzoneInput) {
      el.uploadBtn.addEventListener('click', () => el.dropzoneInput.click());
    }
    if (el.fileList) {
  ...
  function renderFileList() {
    ...
    state.files.forEach(file => {
      ...
      item.className = 'zhida-file-item';

      const meta = document.createElement('div');
      meta.className = 'zhida-file-meta';
      const name = document.createElement('span');
      name.className = 'zhida-file-name';
      name.textContent = file.name;
      const size = document.createElement('span');
      size.className = 'zhida-file-size';
      size.textContent = formatBytes(file.size);
      meta.appendChild(name);
      meta.appendChild(size);

      const status = document.createElement('div');
      status.className = 'zhida-file-status';
      status.textContent = t('ai_qa_file_status_pending');

      const remove = document.createElement('button');
      remove.type = 'button';
      remove.className = 'btn ghost';
      remove.textContent = t('ai_qa_file_remove');
      remove.setAttribute('data-file-remove', file.id);

      const actions = document.createElement('div');
      actions.style.display = 'flex';
      actions.style.alignItems = 'center';
      actions.style.gap = '.65rem';
