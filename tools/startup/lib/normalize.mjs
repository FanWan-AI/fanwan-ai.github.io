...
import { cleanString, cleanStringArray, isoNow, slugify } from "./utils.mjs";
...
export function normalizeOpportunity(raw, index, sourcesById) {
  ...
  const evidence = (Array.isArray(raw?.evidence) ? raw.evidence : []).map((entry) => ({
    claim: cleanString(entry?.claim, 500), source_ids: [...new Set(Array.isArray(entry?.source_ids) ? entry.source_ids.filter((id) => sourcesById.has(id)) : [])], confidence: ["high", "medium", "low"].includes(entry?.confidence) ? entry.confidence : "low"
  })).filter((entry) => entry.claim && entry.source_ids.length);
  const item = {
...
export function buildDailyDocument(raw, sources, date, warnings = []) {
  ...
  return {
    ...
    editorial_note: cleanString(raw?.editorial_note || "本日仅保留能够落到具体客户、证据和最快验证动作的机会。", 1200),
...
export function normalizeCase(raw, sources, { status = "draft", now = isoNow() } = {}) {
  ...
  const sections = (Array.isArray(raw?.sections) ? raw.sections : []).slice(0, MAX_CASE_SECTIONS).map((section, index) => ({
    id: slugify(section?.id || section?.heading || `section-${index + 1}`), heading: cleanString(section?.heading, 160), thesis: cleanString(section?.thesis, 500),
    paragraphs: cleanStringArray(section?.paragraphs, 12, 1800),
    source_ids: [...new Set((Array.isArray(section?.source_ids) ? section.source_ids : []).filter((id) => sourcesById.has(id)))],
    evidence_labels: (Array.isArray(section?.evidence_labels) ? section.evidence_labels : []).map((entry) => ({
      ...
      source_ids: [...new Set((Array.isArray(entry?.source_ids) ? entry.source_ids : []).filter((id) => sourcesById.has(id)))], confidence: ["high", "medium", "low"].includes(entry?.confidence) ? entry.confidence : "low"
    })).filter((entry) => entry.text)
  }));
  return {
    schema_version: SCHEMA_VERSION, kind: "startup_case_analysis", slug: slugify(raw?.slug || raw?.title), status,
    ...
    timeline: (Array.isArray(raw?.timeline) ? raw.timeline : []).slice(0, 16).map((entry) => ({ date: cleanString(entry?.date, 30), event: cleanString(entry?.event, 500), source_ids: [...new Set((Array.isArray(entry?.source_ids) ? entry.source_ids : []).filter((id) => sourcesById.has(id)))] })),
