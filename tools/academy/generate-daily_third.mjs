...
function buildPracticeFallbacks(candidate) {
  ...
  // 从学习目标中提取默认指标；若无则采用通用的“concept clarity”和“application skill”，以避免硬编码时间/质量指标
  const metricPrimary = candidate.learningObjectives?.[0] || "concept clarity";
  const metricSecondary = candidate.learningObjectives?.[1] || "application skill";
  return [
    {
      type: "mcq",
      question: {
