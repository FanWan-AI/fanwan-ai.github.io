...
const LESSON_SYSTEM = "You are the master instructor of AI Daily Academy. You craft bilingual (zh / en, optional es) lessons with a single narrative spine, explicit section transitions, concrete examples, tooling guidance, and high-agency practice.";
...
const BLUEPRINT_OBJECTIVES = `请分阶段构建课程蓝图 JSON，帮助后续生成具有连贯故事线的课件：\n{
  "theme_summary": "一句话说明主题的现实意义，含可量化目标",
  "narrative_spine": {"setup":"学习者当前的认知/痛点","turning_point":"关键洞察或突破事件","payoff":"学习后可衡量的业务结果"},
  "sections": [
    {
      "id": "s1",
      "title": {"zh":"章节名","en":"Section title"},
      "learner_problem": "学员当下面临的业务痛点，写明数据或指标",
      "narrative_arc": "3 句话串联 why → what → how 的故事线",
      "transition_from": "上一节需要延续的悬念或待解问题，首节写 Intro",
      ...
      "worked_example": {"dataset":"使用的数据集或表","fields":["列出字段"],"steps":["列出数学/代码步骤"],"result":"输出的数值/结论","tool":"使用的框架/函数","code":"可选的伪代码或 SQL"},
      "metrics": ["time_to_value","quality_score", "自定义指标"],
      "tools": [{"name":"工具/模板","url":"https://...","usage":"用途"}],
      "practice_hook": ["根据本节可考察的技能点描述 1-2 个题干提示"],
      "visual_hint": "建议的可视化方式"
    }
  ],
  "closing": {"recap":"3 句话总结如何首尾呼应 narrative_spine","next_action":"学习者接下来 72 小时的行动","metrics_handshake":["最重要的 2 个指标"]},
  "references": [{"title":"论文/白皮书","url":"https://...","publisher":"机构","year":2022,"note":"与课程关联"}],
  "glossary": [{"term":"术语","definition":"定义","translation":"英文/中文"}]
}
约束：
1. sections 控制在 4 节（含开篇洞察、案例推导、工具拆解、实战落地），按“痛点→推导→工具→实战”自然推进，可重命名但需体现承上启下；title 必须提供 zh/en，便于后续双语渲染。
...
4. 各节 practice_hook 要与章节内容强关联，便于后续出题，并指明该题需要的素材（数据表、公式、案例）。
5. glossary 需覆盖 2-3 个核心术语，方便跨语言表达。
6. 如 topics 数据或生成脚本已提供 reference hints（推荐文献），请优先引用这些来源，并在 case.citation / references 中准确标注。
7. theme_summary、narrative_spine、closing 必须互相呼应，形成 “开篇设悬念 → 逐节解题 → 收束行动” 的连贯体验。`;

const LESSON_CONTENT_OBJECTIVES = `扩写要求：\n1. summary：zh 需 40-60 字、en 35-60 words，突出“学习前 → 学习后”的变化；若不存在可靠的量化指标，可用定性描述，并在 en 中自然翻译。\n2. content：
   - 以 <p class="narrative-intro"> 段落串起 blueprint.theme_summary 与 narrative_spine.setup。
   - 按 blueprint.sections 顺序输出 4 个 <section data-section="sX">，每节结构：
     * <h3> 结合章节标题 + 关键指标；
     * <p class="section-bridge"> 概述上一节遗留的问题（使用 transition_from）；首节说明为何此主题迫切；
     * 2-3 段 “痛点→策略→结果” 叙事，并在 zh/en 中引用对应的 case.citation（格式 "[引用标题]"）；
     * 一个来自 worked_example 的公式或代码块（<pre><code> 或 <table>），解释每个字段的含义；
     * <ol> 三步执行计划，显式写出如何监控 time_to_value / quality_score 或其它指标；
     * <p class="callout"> 提炼本节适用场景或工具。
   - 以 <p class="narrative-outro"> 收束，呼应 narrative_spine.payoff 与 closing.next_action。
3. 至少 1 节需完整演示 worked example：列出数据字段、计算步骤和最终结果，可包含 Python/SQL，并解释结果与业务痛点的关系。
4. content.en 必须是流畅英文，允许与中文结构相同但措辞自然；若需引用中文术语，请改写为 "self-attention (自注意力)" 形式，禁止出现连续中文字符。若模型无法提供 es，可省略。
5. references 只能引用 blueprint.references，保持标题、出版年份一致；如确无真实来源则返回 Internal insight。严禁新增虚构资料。
6. 本阶段不要生成 practice，留给后续步骤完成；若字段不可避免，请设为空数组。
7. meta.contextual_notes 继承 blueprint.contextual_hooks，并补充 worked example 的可视化方式（如血缘图/折线图等）。
8. 在所有叙述和公式中，如果没有可信数据支撑，请不要编造数值或百分比提升，可用“显著提高”“明显减少”等描述。
9. 使用中英双语术语（如“自注意力 self-attention”），并确保每节之间有承上启下的桥段，避免割裂。`;
...
const CRITIC_OBJECTIVES = `请审阅 lesson 与 blueprint/learner_profile 的匹配度，给出 JSON：\n{"revision_required": bool, "scorecard": {"structure":{"score":0-5,"notes":""},"accuracy":{"score":0-5,"notes":""},"depth":{"score":0-5,"notes":""},"practice":{"score":0-5,"notes":""},"references":{"score":0-5,"notes":""},"cohesion":{"score":0-5,"notes":""}}, "issues": [{"area":"content|practice|references|metrics|depth|cohesion","severity":"high|medium|low","note":"问题描述","action":"建议"}], "strengths": ["亮点..."], "directives": ["按优先级列出需改进的明确 TODO"], "practice_expectations": {"min_questions":4,"required_types":["mcq","multi","input"],"require_data_driven":true}, "content_expectations": {"require_worked_example":true,"require_formula":true,"require_steps":true,"min_sections":4,"require_section_bridges":true,"require_intro_outro":true}}。若任意节缺少数字示例/公式/<pre><code>/步骤、或 content 未按 data-section 包裹、或缺失 narrative intro/outro，请将 revision_required 设为 true 并给出整改建议。若无问题，可设置 revision_required=false 且 issues 为空。`;
...
约束：
  ...
  2. 每道题都要指出 source_section，与正文保持一致，并在题干中点明“来自 <section 标题>”。
